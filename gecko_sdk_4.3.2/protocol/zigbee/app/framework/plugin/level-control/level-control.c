...
static void moveToLevelHandler(uint8_t commandId, uint8_t level,
                               uint16_t transitionTimeDs, uint8_t optionMask,
...
static void stepHandler(uint8_t commandId, uint8_t stepMode, uint8_t stepSize,
                        uint16_t transitionTimeDs, uint8_t optionMask,
                        uint8_t optionOverride);
static void stopHandler(uint8_t commandId, uint8_t optionMask,
                        uint8_t optionOverride);
...
void emberAfLevelControlClusterServerTickCallback(uint8_t endpoint)
{
  ...
  if (currentLevel == state->moveToLevel)
  {
    ...
    else
    {
      ...
      {
        emberAfLevelControlClusterPrintln("restoring current level to %d",
                                          state->storedLevel);
...
static void writeRemainingTime(uint8_t endpoint, uint16_t remainingTimeMs)
{
  ...
  EmberStatus status = emberAfWriteServerAttribute (
      endpoint,
      ZCL_LEVEL_CONTROL_CLUSTER_ID,
...
static void setOnOffValue(uint8_t endpoint, bool onOff)
{
  if (emberAfContainsServer (endpoint, ZCL_ON_OFF_CLUSTER_ID))
  {
    emberAfLevelControlClusterPrintln(
        "Setting on/off to %p due to level change", onOff ? "ON" : "OFF");
    emberAfOnOffClusterSetValueCallback (
        endpoint, (onOff ? ZCL_ON_COMMAND_ID : ZCL_OFF_COMMAND_ID),
        true);
  }
}
                               ...
                               uint8_t optionMask, uint8_t optionOverride)
{
  ...
  ZCL_ON_OFF_CLUSTER_ID,
                                       ZCL_ON_OFF_ATTRIBUTE_ID, (uint8_t*) &on,
                                       sizeof(on));
  if (status != EMBER_ZCL_STATUS_SUCCESS)
  {
  ...
  if (READBITS(optionMask, EMBER_ZCL_LEVEL_CONTROL_OPTIONS_EXECUTE_IF_OFF))
  {
    ...
    return READBITS(optionOverride,
...
bool emberAfLevelControlClusterMoveToLevelCallback(EmberAfClusterCommand *cmd)
{
  ...
  emberAfLevelControlClusterPrintln("%pMOVE_TO_LEVEL %x %2x %x %x",
                                    "RX level-control:", cmd_data.level,
                                    cmd_data.transitionTime,
                                    cmd_data.optionMask,
                                    cmd_data.optionOverride);
  moveToLevelHandler (ZCL_MOVE_TO_LEVEL_COMMAND_ID, cmd_data.level,
                      cmd_data.transitionTime, cmd_data.optionMask,
                      cmd_data.optionOverride,
    ...
    EmberAfClusterCommand *cmd)
{
  ...
  if (zcl_decode_level_control_cluster_move_to_level_with_on_off_command (
      cmd, &cmd_data) != EMBER_ZCL_STATUS_SUCCESS)
  {
    return false;
  }
  ...
  emberAfLevelControlClusterPrintln("%pMOVE_TO_LEVEL_WITH_ON_OFF %x %2x",
                                    "RX level-control:", cmd_data.level,
                                    cmd_data.transitionTime);
  moveToLevelHandler (ZCL_MOVE_TO_LEVEL_WITH_ON_OFF_COMMAND_ID, cmd_data.level,
                      cmd_data.transitionTime, 0xFF, 0xFF,
                      INVALID_STORED_LEVEL); // Don't revert to the stored level
  return true;
}
...
bool emberAfLevelControlClusterMoveCallback(EmberAfClusterCommand *cmd)
{
  ...
  emberAfLevelControlClusterPrintln("%pMOVE %x %x", "RX level-control:",
                                    cmd_data.moveMode, cmd_data.rate);
  moveHandler (ZCL_MOVE_COMMAND_ID, cmd_data.moveMode, cmd_data.rate,
               cmd_data.optionMask, cmd_data.optionOverride);
  return true;
}
...
bool emberAfLevelControlClusterMoveWithOnOffCallback(EmberAfClusterCommand *cmd)
{
  ...
  emberAfLevelControlClusterPrintln("%pMOVE_WITH_ON_OFF %x %x",
                                    "RX level-control:", cmd_data.moveMode,
                                    cmd_data.rate);
  moveHandler (ZCL_MOVE_WITH_ON_OFF_COMMAND_ID, cmd_data.moveMode,
               cmd_data.rate, 0xFF, 0xFF);
  return true;
}
...
bool emberAfLevelControlClusterStepCallback(EmberAfClusterCommand *cmd)
{
  ...
  emberAfLevelControlClusterPrintln("%pSTEP %x %x %2x", "RX level-control:",
                                    cmd_data.stepMode, cmd_data.stepSize,
                                    cmd_data.transitionTime);
  stepHandler (ZCL_STEP_COMMAND_ID, cmd_data.stepMode, cmd_data.stepSize,
               cmd_data.transitionTime, cmd_data.optionMask,
               cmd_data.optionOverride);
  return true;
}
...
bool emberAfLevelControlClusterStepWithOnOffCallback(EmberAfClusterCommand *cmd)
{
  ...
  emberAfLevelControlClusterPrintln("%pSTEP_WITH_ON_OFF %x %x %2x",
                                    "RX level-control:", cmd_data.stepMode,
                                    cmd_data.stepSize, cmd_data.transitionTime);
  stepHandler (ZCL_STEP_WITH_ON_OFF_COMMAND_ID, cmd_data.stepMode,
               cmd_data.stepSize, cmd_data.transitionTime, 0xFF, 0xFF);
  return true;
}
...
bool emberAfLevelControlClusterStopCallback(EmberAfClusterCommand *cmd)
{
  ...
  stopHandler (ZCL_STOP_COMMAND_ID, cmd_data.optionMask,
               cmd_data.optionOverride);
  return true;
}
                               ...
                               uint8_t optionOverride, uint16_t storedLevel)
{
  ...
  if (transitionTimeDs == 0xFFFF)
  {
    ...
    status = emberAfReadServerAttribute (
        endpoint,
        ZCL_LEVEL_CONTROL_CLUSTER_ID,
        ZCL_ON_OFF_TRANSITION_TIME_ATTRIBUTE_ID, (uint8_t*) &transitionTimeDs,
        sizeof(transitionTimeDs));
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
      emberAfLevelControlClusterPrintln(
          "ERR: reading on/off transition time %x", status);
      goto send_default_response;
    }
    ...
    state->transitionTimeMs = (transitionTimeDs * MILLISECOND_TICKS_PER_SECOND
        / 10);
...
send_default_response: if (emberAfCurrentCommand()->apsFrame->clusterId
      == ZCL_LEVEL_CONTROL_CLUSTER_ID)
  {
                        ...
                        uint8_t optionMask, uint8_t optionOverride)
{
  ...
  ZCL_ON_OFF_CLUSTER_ID,
                                 ...
                                 CLUSTER_MASK_SERVER,
                                 &on_off_state, sizeof(on_off_state),
                                 NULL);
  if (status != EMBER_ZCL_STATUS_SUCCESS)
  {
    emberAfLevelControlClusterPrintln("ERR: reading onoff %x", status);
    goto send_default_response;
  }
  ...
  switch (moveMode)
  {
    case EMBER_ZCL_MOVE_MODE_UP:
      state->increasing = true;
      state->moveToLevel = MAX_LEVEL;
      if (commandId == ZCL_MOVE_WITH_ON_OFF_COMMAND_ID && on_off_state == 0x00)
      {
        ...
        {
          emberAfLevelControlClusterPrintln(
              "ERR: unable to set startup level %x", status);
        }
      }
...
send_default_response: emberAfSendImmediateDefaultResponse (status);
}
                        ...
                        uint8_t optionOverride)
{
  ...
  {
    status = EMBER_ZCL_STATUS_SUCCESS;
    emberAfLevelControlClusterPrintln("Skip step\n");
    goto send_default_response;
  }
  ...
  switch (stepMode)
  {
    case EMBER_ZCL_STEP_MODE_UP:
      ...
      if (commandId == ZCL_STEP_WITH_ON_OFF_COMMAND_ID)
      {
        ...
        status = emberAfReadServerAttribute (endpoint,
        ZCL_ON_OFF_CLUSTER_ID,
                                             ZCL_ON_OFF_ATTRIBUTE_ID, &on_off,
                                             sizeof(on_off));
        if (status == EMBER_ZCL_STATUS_SUCCESS && on_off == 0x00)
        {
          /* starting transition, so set currentLevel to minimum */
          currentLevel = MIN_LEVEL;
  ...
  if (currentLevel <= state->moveToLevel)
  {
    ...
    if (currentLevel == state->moveToLevel)
    {
      status = EMBER_ZCL_STATUS_SUCCESS;
      emberAfLevelControlClusterPrintln(
          "Skip step. Current level(%d) == moveToLevel", (currentLevel));
      writeRemainingTime (endpoint, 0);
      goto send_default_response;
    }
  }
...
bool newValue)
{
  ...
  if (newValue)
  {
    ...
    status = emberAfWriteServerAttribute (
        endpoint,
        ZCL_LEVEL_CONTROL_CLUSTER_ID,
        ZCL_CURRENT_LEVEL_ATTRIBUTE_ID,
    ...
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
      emberAfLevelControlClusterPrintln("ERR: reading current level %x",
                                        status);
      return;
    }
    ...
    moveToLevelHandler (ZCL_MOVE_TO_LEVEL_COMMAND_ID, resolvedLevel,
                        currentOnOffTransitionTime, 0xFF, 0xFF,
...
void emberAfLevelControlClusterServerInitCallback(uint8_t endpoint)
{
  ...
  if (areStartUpLevelControlServerAttributesTokenized (endpoint))
  {
    ...
    EmberAfStatus status = emberAfReadAttribute (
        endpoint,
        ZCL_LEVEL_CONTROL_CLUSTER_ID,
        ZCL_START_UP_CURRENT_LEVEL_ATTRIBUTE_ID,
        CLUSTER_MASK_SERVER,
        (uint8_t*) &startUpCurrentLevel, sizeof(startUpCurrentLevel),
        NULL);
    if (status == EMBER_ZCL_STATUS_SUCCESS)
    {
...
static bool areStartUpLevelControlServerAttributesTokenized(uint8_t endpoint)
{
  ...
  metadata = emberAfLocateAttributeMetadata (
      endpoint,
      ZCL_LEVEL_CONTROL_CLUSTER_ID,
      ZCL_START_UP_CURRENT_LEVEL_ATTRIBUTE_ID,
      CLUSTER_MASK_SERVER,
...
uint32_t emberAfLevelControlClusterServerCommandParse(
    sl_service_opcode_t opcode, sl_service_function_context_t *context)
{
  (void) opcode;
  ...
  return (
      (wasHandled) ? EMBER_ZCL_STATUS_SUCCESS : EMBER_ZCL_STATUS_UNSUP_COMMAND);
}
